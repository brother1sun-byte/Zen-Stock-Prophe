## 追加実装: P2.4 リリースノートと最終QA記録

P2.4では、新機能を増やさず、完成版として提出・共有できる状態にするための証跡を整備した。

- `docs/release-notes.md` を追加し、リリース概要、アプリの目的、完成した主な機能、安全設計、データソース、API未設定時の挙動、未取得データ、ChatGPT相談用プロンプト、外部AI送信なし、実注文機能なし、証券会社API未接続、TDnet相当データ、テスト結果、未対応項目を整理した。
- `docs/final-qa-report.md` を追加し、対象コミット、確認コマンド、主要画面、CSVインポート、EDINET、J-Quants、TDnet相当データ、ChatGPT相談用プロンプト、重要材料サマリー、安全境界を確認できるようにした。
- ChatGPT APIへ直接送信しないこと、APIキーを表示しないこと、実注文機能なし、証券会社API未接続をP2.4の安全境界として明示した。
- README冒頭から操作マニュアル、リリース前チェックリスト、リリースノート、最終QA記録、サンプルCSVへ辿れるようにした。
- リリース前チェックリストに、リリースノート確認、最終QA記録確認、README導線、ドキュメント間の整合確認を追加した。
...
## 追加実装: P2.3 操作マニュアルとUI最終微調整

P2.3では、新しい外部連携を増やさず、初回利用者が迷わず使える導線を整備した。

- `docs/user-manual.md` を追加し、目的、できること、できないこと、初回セットアップ、API設定、CSVインポート、寄り付き前チェック、開示・決算チェック、ウォッチリスト一括チェック、重要材料サマリー、ChatGPT相談用プロンプト、TDnet相当データ、エラー表示を短文で整理した。
- README冒頭に初回利用の流れを追加し、操作マニュアル、サンプルCSV/JSON、リリース前チェックリストへ辿れるようにした。
- 画面のデータ設定パネルに「初回利用の流れ」を追加し、操作マニュアル、サンプルCSV、リリース前チェックリストへの導線をまとめた。
- `docs/release-checklist.md` を補強し、操作マニュアル、README導線、プロンプトコピー、禁止表現、APIキー非表示、外部AI送信なしを確認できるようにした。

現時点の完成範囲:

- 日本株リサーチ用の朝チェック導線
- EDINET、J-Quants、営業日カレンダー、TDnet相当データ状態の見える化
- ウォッチリストCSVインポート
- ウォッチリスト一括チェック
- 根拠付きリサーチ要約
- ChatGPT相談用コピー作成
...
## 追加実装: P2.2 UI最終整理・設定画面・サンプルデータ・リリース前チェックリスト

P2.2では、新しい外部データ取得へ進まず、完成版として見せられる状態を整えた。

- ウォッチリスト画面に「データ設定」パネルを追加し、EDINET、J-Quants、TDnet相当データ、日本祝日データ、手動データ、キャッシュ、ChatGPT連携、実注文、証券会社APIの状態を一覧化した。
- APIキーそのものは表示せず、設定済み、未設定、認証失敗、取得失敗、データなし、データ未取得、キャッシュ利用、手動データ、照合不可、未実装、一部未取得などの状態名へ寄せた。
- `docs/samples/` にウォッチリストCSV、異常系CSV、手動決算予定JSON、手動TDnet相当JSONのサンプルを追加した。
- `docs/release-checklist.md` を追加し、起動、主要画面、CSVインポート、API未設定、手動データ、キャッシュ、ウォッチリスト一括チェック、ChatGPT相談用プロンプト、重要材料サマリー、検証コマンドを公開前に確認できるようにした。
...
- CSVウォッチリストインポート
- TDnet相当データの未取得明示
- データ設定パネル、サンプルデータ、リリース前チェックリスト

未対応範囲:

- TDnet公式APIまたはJ-Quantsアドオンによる実取得
- 手動データを画面上で編集する機能
- キャッシュ管理画面
- リリースノート
- 操作マニュアル
