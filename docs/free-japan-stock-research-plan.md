## 追加実装: P2.1 CSVインポートとTDnet相当データ整理

P2.1として、ウォッチリストCSVインポートとTDnet相当データの安全な状態表示を実装した。目的は、ユーザー自身の監視銘柄を朝の一括チェック、重要材料サマリー、ChatGPT相談用プロンプトへ自然につなげることである。

- `src/utils/watchlistCsvImporter.js` を追加し、CSV解析、銘柄コード正規化、不正行検出、重複統合、インポートプレビューを純粋関数として分離した。
- ウォッチリスト画面にCSVファイル選択、CSV貼り付け欄、プレビュー、スキップ理由、重複件数、追加ボタンを追加した。
- インポート後の銘柄は、ウォッチリスト一覧、ウォッチリスト一括寄り付き前チェック、重要材料サマリー、ChatGPT相談用プロンプト生成に反映される。
- `src/utils/tdnetSourceStatus.js` と `src/data/manualTdnetEvents.json` を追加し、TDnet相当データの未取得、手動データ、将来アドオン利用余地を明示できるようにした。
- TDnet相当データは公式APIまたはJ-Quantsアドオンなどの正規ルートを前提に慎重に扱う。無料前提では規約リスクのあるスクレイピングを行わない。

無料データソースでできる範囲は、EDINET、J-Quants決算予定、日本営業日カレンダー、手元CSV、キャッシュ、手動JSONの整理までである。TDnet相当データは、現段階では未取得として扱い、不足情報と一次情報確認の対象に残す。
...
## 追加実装: P1.9 重要材料の要約と根拠表示

P1.9として、EDINET提出書類、J-Quants決算予定、日本営業日カレンダー、寄り付き前チェック、ウォッチリスト一括チェック、データ取得状況をもとに、重要材料サマリーを画面上に表示する機能を実装した。

- `src/utils/researchInsightBuilder.js` を追加し、単一銘柄とウォッチリスト全体の材料サマリー、強材料、弱材料、注意点、不足情報、根拠、データ充足度スコアを純粋関数として分離した。
- この表示はAI分析ではなく、既存データからのルールベース材料整理である。外部LLMやChatGPT APIへ直接送信しない。
- 信頼度スコアは投資判断の確度ではなく、EDINET、J-Quants、日本営業日、TDnet相当、キャッシュ、手動データ、API未設定、取得失敗を踏まえたデータ充足度として扱う。
- P1.8のChatGPT相談用プロンプトにも、重要材料サマリー、根拠、不足情報を含めるようにした。

次工程はP2として、CSVインポート、TDnet相当データの安全な扱い、UI仕上げ、画面回帰テスト強化へ進む。

## 追加実装: P1.8 ChatGPT相談用プロンプト生成

P1.8として、EDINET提出書類、J-Quants決算予定、日本営業日カレンダー、寄り付き前チェック、ウォッチリスト一括チェック結果をまとめ、ChatGPTへ貼り付けるための相談用プロンプトを自動生成できるようにした。

- `src/utils/chatGptPromptBuilder.js` を追加し、単一銘柄用、ウォッチリスト用、EDINET整形、決算予定整形、寄り付き前チェック整形、未取得データ注意文を分離した。
- ウォッチリスト一括チェック画面に「ChatGPT相談用プロンプト」を追加し、作成、表示、コピー、コピー失敗時の手動コピー導線を用意した。
- ChatGPT APIへ直接送信しない。理由は、P1.8では認証・課金・外部送信リスクを増やさず、ユーザーが内容を確認してから貼り付ける安全なリサーチ補助に留めるため。
- プロンプトには、API未設定、取得失敗、キャッシュ利用、手動データ利用、祝日データ状態などを明示し、一次情報確認の前提を崩さない。
- 次工程はP1.9「重要材料のAI要約と根拠表示」。ただしP1.9では、根拠リンクと取得状態を必ず併記し、投資助言ではなく材料確認に限定する。
...
## 追加実装: ウォッチリスト寄り付き前チェック

P1.6として、単一銘柄の寄り付き前チェックをウォッチリスト全体へ拡張した。

### 実装内容

- `src/utils/watchlistPreopenCheck.js` を追加し、ウォッチリスト銘柄の寄り付き前材料確認、件数サマリー、注意度順の並び替え、フィルター、ChatGPT相談用材料生成の土台を実装。
- EDINET、決算カレンダー、日本営業日カレンダーの同じ取得結果を再利用し、銘柄ごとに外部APIを連打しない設計にした。
- ウォッチリスト画面に「ウォッチリスト一括チェック」を追加し、対象銘柄数、確認済み、重要予定あり、確認推奨、データ未取得、目立つ材料なし、エラー件数を表示。
- 銘柄別にEDINET提出書類、決算予定、照合方法、ソース別取得状況、一次情報確認メッセージを展開できるようにした。

## 追加実装: J-Quants決算予定実取得

P1.7として、決算カレンダー土台をJ-Quants V2の決算発表予定日APIへ接続した。認証情報はFastAPI側の `JQUANTS_API_KEY` だけで扱い、ブラウザへ露出しない。

### 実装内容

- `backend/earnings_calendar_service.py` を拡張し、`/api/research/earnings-calendar` からJ-Quants `GET /v2/equities/earnings-calendar` を読み取り専用で確認する。
- J-Quants実取得、API未設定、認証失敗、取得失敗、データなし、手動データ、キャッシュ利用を別ステータスとして返す。
- 手動JSONとキャッシュはフォールバックとしてのみ使い、画面では「手動データ」「キャッシュ利用」と明示する。
- `src/utils/earningsCalendarClient.js` でJ-Quants由来の決算予定を正規化し、寄り付き前チェック、開示・決算チェック、ウォッチリスト一括チェックへ同じ取得結果を流す。
- 今日または次営業日の決算予定は「重要予定あり」、近日中またはフォールバック確認は「確認推奨」として扱う。ただし売買推奨ではなく一次情報確認の補助に限定する。

### 残る制約

- J-Quants側のAPI仕様、提供範囲、契約プラン、更新タイミングによりデータが返らない場合がある。
- TDnet相当データは引き続き有料・規約面の制約があるため、規約リスクのあるスクレイピングは行わない。
- 無料または低コストのデータ源だけでは開示・決算材料の網羅性に限界がある。

### 次の改善候補

- P1.8: ChatGPT相談用プロンプトへ開示・決算材料を自動挿入する。
- P1.9: 重要材料のAI要約と根拠表示を追加する。
- ウォッチリストCSVインポートとTDnetアドオン利用時の安全設計を検討する。
