...
async function mockApi(page, source = 'synthetic', options = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path.includes('/research/earnings-calendar')) {
      ...
      return route.fulfill({
        json: {
          status: items.length ? 'success' : 'api_key_missing',
          startDate: params.get('start_date') || '2026-06-26',
          endDate: params.get('end_date') || '2026-06-29',
          fetchedAt: '2026-06-26T23:35:00Z',
          items,
          sourceStatus: items.length
            ? { label: options.earningsSourceLabel || '手動データ', tone: 'warn', detail: '手動JSONの決算予定を表示しています。一次情報を確認してください。' }
            : { label: 'J-Quants API未設定', tone: 'warn', detail: '決算予定データは未取得です。' },
          message: items.length ? '決算予定を取得しました。' : '決算予定データは未取得です。',
        },
      });
    }
