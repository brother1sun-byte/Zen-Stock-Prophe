...
async function mockApi(page, source = 'synthetic', options = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path.includes('/portfolio/positions/') && path.endsWith('/lifecycle')) {
      lifecycleSaved = true;
      return route.fulfill({ json: { message: '練習用の台帳状態を更新しました。' } });
    }
    ...
    if (path.endsWith('/portfolio/positions')) {
      positionSaved = true;
      return route.fulfill({ json: { message: '練習注文を台帳へ保存しました。' } });
    }
    if (path.endsWith('/portfolio')) {
      if ((positionSaved || lifecycleSaved) && options.failPortfolioRefresh) {
        return route.fulfill({ status: 503, json: { detail: '台帳の再読込に失敗しました。' } });
      }
      const portfolio = lifecycleSaved
        ? (options.portfolioAfterLifecycleSave || options.portfolio || data.portfolio)
        : positionSaved
          ? (options.portfolioAfterPositionSave || options.portfolio || data.portfolio)
          : (options.portfolio || data.portfolio);
      return route.fulfill({ json: portfolio });
    }
    if (path.endsWith('/transactions')) {
      if ((positionSaved || lifecycleSaved) && options.failTransactionRefresh) {
        return route.fulfill({ status: 503, json: { detail: '操作履歴の再読込に失敗しました。' } });
      }
      const transactions = lifecycleSaved
        ? (options.transactionsAfterLifecycleSave || options.transactions || [])
        : positionSaved
          ? (options.transactionsAfterPositionSave || options.transactions || [])
...
test('永続化された保有履歴と操作履歴を再読み込み後も表示する', async ({ page }) => {
  const archivedHolding = {
    ticker: '4980.T',
    name: 'デクセリアルズ',
    shares: 120,
    avgCost: 2481,
    ...
    pnlPct: 727.0,
    status: 'SOLD',
    lifecycleReason: '利益確定',
    lifecycleUpdatedAt: '2026-07-13T10:15:00+09:00',
  };
  const transaction = {
    id: 243,
    ticker: '6309.T',
    ...
    total: 499110,
    reason: '練習注文',
    createdAt: '2026-07-13T10:20:00+09:00',
  };
  const base = mockPayload('yfinance');
  await mockApi(page, 'yfinance', {
    portfolio: { ...base.portfolio, archivedHoldings: [archivedHolding] },
    transactions: [transaction],
  });
  await page.goto('/');
  await page.locator('.detail-toggle').click();

  await expect(page.getByTestId('portfolio-ledger-events')).toContainText('4980.T');
  await expect(page.getByTestId('portfolio-ledger-events')).toContainText('売却済み');
  await expect(page.getByTestId('portfolio-transaction-history')).toContainText('6309.T');
  await expect(page.getByTestId('portfolio-transaction-history')).toContainText('262株');
});

test('練習注文の保存直後に保有と操作履歴へ反映する', async ({ page }) => {
  const base = mockPayload('yfinance');
  const savedHolding = {
    ticker: base.ticker,
  ...
  await mockApi(page, 'yfinance', {
    ...
    portfolioAfterPositionSave: { ...base.portfolio, holdings: [savedHolding] },
    transactionsAfterPositionSave: [savedTransaction],
  });
  await page.goto('/');

  await page.locator('[data-testid="practice-order-price"]:visible').fill('1905');
  await page.locator('[data-testid="practice-order-shares"]:visible').fill('262');
  await page.locator('[data-testid="practice-order-save"]:visible').click();

  await expect(page.getByTestId('practice-position-list')).toContainText(`${base.ticker}`);
  await expect(page.getByTestId('practice-position-list')).toContainText('262株');
  await page.locator('.detail-toggle').click();
  await expect(page.getByTestId('portfolio-transaction-history')).toContainText(`${base.ticker} 262株`);
});
...
test('バックグラウンド更新は重い市場APIを再取得しない', async ({ page }) => {
  await page.addInitScript(() => {
  ...
  const heavyRequestCounts = new Map([
    ...
    ['/api/research/jquants/status', 0],
  ]);
  page.on('request', (request) => {
    const path = new URL(request.url()).pathname;
    for (const endpoint of heavyRequestCounts.keys()) {
      if (path.includes(endpoint)) {
        heavyRequestCounts.set(endpoint, heavyRequestCounts.get(endpoint) + 1);
      }
    }
  });
  await mockApi(page, 'yfinance');
  ...
  for (const count of heavyRequestCounts.values()) expect(count).toBe(1);
});
...
test('練習注文の保存後に台帳再読込が失敗しても約定済みを維持する', async ({ page }) => {
  await mockApi(page, 'yfinance', { failPortfolioRefresh: true, failTransactionRefresh: true });
  await page.goto('/');
  ...
  await expect(history).not.toContainText('保存失敗');
  await expect(page.locator('.ops-panel .log-feed')).toContainText('保存済みですが、最新の台帳表示を取得できませんでした');
});

test('生活導線4モードで手入力価格と引け後レビューを確認できる', async ({ page }) => {
  await mockApi(page, 'yfinance');
  await page.goto('/');

  const panel = page.getByTestId('lifestyle-daytrade-panel');
  await expect(panel).toBeVisible();
  ...
  await expect(page.getByTestId('lifestyle-morning-card')).toContainText('注文上限との差');
  await expect(page.getByTestId('lifestyle-morning-card')).toContainText('撤退ライン');
  await expect(page.getByTestId('lifestyle-morning-card')).toContainText('ウォークフォワード');

  await page.getByTestId('lifestyle-mode-work').click();
  await expect(page.getByTestId('lifestyle-work-monitor')).toBeVisible();
  await expect(page.getByTestId('lifestyle-work-monitor')).toContainText(/保有継続|利確検討|撤退検討/);

  await page.getByTestId('lifestyle-mode-review').click();
  await expect(page.getByTestId('lifestyle-after-close-review')).toBeVisible();
  await expect(page.getByTestId('after-close-review-json')).toContainText('判断材料');
  await page.getByTestId('after-close-review-ticker').fill('7203.T');
  await page.getByTestId('after-close-review-entryPrice').fill('3000');
  await page.getByTestId('after-close-review-exitPrice').fill('2980');
  await page.getByTestId('after-close-review-shares').fill('100');
  await page.getByTestId('after-close-review-initialScore').fill('82');
  await page.getByTestId('after-close-review-missedSignal').fill('VWAP割れ後も保有継続した');
  await page.getByTestId('after-close-review-improvementMemo').fill('次回は撤退ライン接近を早めに確認');
  await page.getByTestId('after-close-review-save').click();
