...
test('データ設定パネルでAPI状態とサンプル導線を確認できる', async ({ page }) => {
  ...
  await expect(panel).toContainText('初回利用の流れ');
  await expect(panel).toContainText('操作マニュアルを確認する');
  await expect(panel).toContainText('ChatGPT相談用プロンプトをコピーする');
  await expect(page.getByTestId('sample-data-links')).toContainText('docs/samples/watchlist-sample.csv');
  await expect(page.getByTestId('sample-data-links')).toContainText('docs/user-manual.md');
