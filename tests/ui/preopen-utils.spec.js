...
test('J-Quants実取得と失敗ステータスを日本語で安全に返す', () => {
  expect(getEarningsCalendarSourceStatus({ status: 'success', items: [{ code: '4980' }] }).label).toBe('J-Quants実取得済み');
  expect(getEarningsCalendarSourceStatus({ status: 'auth_failed', items: [] }).label).toBe('認証失敗');
  expect(getEarningsCalendarSourceStatus({ status: 'fetch_failed', items: [] }).label).toBe('取得失敗');
  expect(getEarningsCalendarSourceStatus({ status: 'cache_used', items: [{ code: '4980' }] }).label).toBe('キャッシュ利用');
});
...
test('次営業日のJ-Quants決算予定は重要予定として扱う', () => {
  const businessWindow = buildMorningCheckWindow(new Date('2026-06-26T08:30:00+09:00'));
  const summary = buildPreopenCheckSummary({
    stock: { ticker: '4980.T' },
    businessWindow,
    earningsCalendar: {
      status: 'success',
      items: [{ Code: '49800', CompanyName: 'Dexerials', Date: businessWindow.nextBusinessDay, source: 'J-Quants' }],
    },
    disclosureSummary: { risk: 'low', edinetMeta: { status: 'success' } },
  });
  expect(summary.status).toBe('重要予定あり');
  expect(summary.risk).toBe('high');
});

test('ウォッチリスト一括チェックにJ-Quants決算予定と取得失敗を反映できる', () => {
  const businessWindow = buildMorningCheckWindow(new Date('2026-06-29T08:30:00+09:00'));
  const results = buildWatchlistPreopenCheck([
    ...
    earningsCalendar: {
      status: 'success',
      items: [{ Code: '49800', CompanyName: 'Dexerials', Date: '2026-06-29', source: 'J-Quants' }],
      sourceStatus: { label: 'J-Quants実取得済み', tone: 'good', detail: 'J-Quantsから取得しました。' },
    },
    edinetDisclosure: { status: 'success', documents: [], morningCheck: businessWindow },
  });
  expect(results[0].ticker).toBe('4980.T');
  expect(results[0].sourceStatus.earnings.label).toBe('J-Quants実取得済み');

  const missing = buildWatchlistPreopenCheck([{ ticker: '7203.T', name: 'トヨタ自動車' }], {
    businessWindow,
    earningsCalendar: {
      status: 'fetch_failed',
      items: [],
      sourceStatus: { label: '取得失敗', tone: 'warn', detail: '通信エラー' },
    },
  });
  expect(missing[0].status).toBe('データ未取得');
  expect(missing[0].sourceStatus.earnings.label).toBe('取得失敗');
});
