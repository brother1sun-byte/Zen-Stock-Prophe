...
class ScreeningEngineTests(unittest.TestCase):
    ...
    def test_prophet_pick_is_canonical_watchlist_candidate(self):
        self.assertIn("6503.T", server.MUST_INCLUDE)
        self.assertIn("6503.T", server.STOCKS)
        self.assertEqual(server.MUST_INCLUDE["6503.T"]["candidate_rank"], 1)
        self.assertEqual(server.MUST_INCLUDE["6503.T"]["candidate_score"], 93.51)

    def test_publish_watchlist_preserves_prophet_pick_first(self):
        ...
        self.assertEqual(published["6503.T"]["candidate_rank"], 1)
        self.assertEqual(published["4980.T"]["candidate_rank"], 2)
        self.assertEqual(published["7203.T"]["candidate_rank"], 3)
    ...
    def test_stock_payload_uses_live_history_score_for_pinned_candidate(self):
        ...
        try:
            ...
            payload = server._stock_payload("6503.T", server.MUST_INCLUDE["6503.T"])
        finally:
            server.get_stock_data = original_get_stock_data

        self.assertNotEqual(payload["candidateScore"], server.MUST_INCLUDE["6503.T"]["candidate_score"])
        self.assertEqual(payload["publishedCandidateScore"], server.MUST_INCLUDE["6503.T"]["candidate_score"])
